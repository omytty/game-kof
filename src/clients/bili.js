(function(window, document) {
    ...
    function bili(id, layer, images, direction, name, reload) {
        ...
        this.defaultImageType = constants.IMAGE_TYPE_STAY;
        this.baseImage = this.images[this.name + '_'+ constants.IMAGE_TYPE_WALK +'_0'];
    ...
    bili.prototype.update = function(timestamp) {
        if (this.isOvertime(timestamp, this.reload)) {
            ...
            var isNotDefaultImageType = (this.state.imageType != this.defaultImageType) && (this.state.imageType != constants.IMAGE_TYPE_WALK);
    ...
    bili.prototype.render = function() {
        ...
        var wh_scale = image.width / image.height;
        var fixedBase = image.height / this.baseImage.height;
        var height = (app.height * 2 / 7) * fixedBase;
        var width = height * wh_scale;
        this.layer.drawImage(image, this.state.X, this.state.Y, width, height);
    };
    ...
    bili.prototype.handleDirection = function(type) {
        ...
        this.setStates({
            speed: speed,
            direction: direction,
            imageType: constants.IMAGE_TYPE_WALK,
            imageIndex: 0,
        });
    };
